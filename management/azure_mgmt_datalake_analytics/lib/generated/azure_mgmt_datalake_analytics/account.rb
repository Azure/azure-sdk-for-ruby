...
module Azure::ARM::DataLakeAnalytics
  ...
  class Account
    ...
    def check_name_availability_async(location, parameters, custom_headers = nil)
      ...
      # Serialize Request
      request_mapper = Azure::ARM::DataLakeAnalytics::Models::CheckNameAvailabilityParameters.mapper()
      ...
      promise = promise.then do |result|
        ...
        if status_code == 200
          begin
            parsed_response = response_content.to_s.empty? ? nil : JSON.load(response_content)
            result_mapper = Azure::ARM::DataLakeAnalytics::Models::NameAvailabilityInformation.mapper()
            result.body = @client.deserialize(result_mapper, parsed_response)
          rescue Exception => e
            fail MsRest::DeserializationError.new('Error occurred in deserializing the response', e.message, e.backtrace, result)
          end
