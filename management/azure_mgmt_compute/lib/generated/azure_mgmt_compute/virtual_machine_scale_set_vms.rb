      ...
      promise = promise.then do |result|
        ...
        if status_code == 200
          begin
            ...
            result.body = Azure::ARM::Compute::Models::VirtualMachineScaleSetVM.new.from_json(parsed_response)
          rescue Exception => e
    ...
    def get_instance_view_async(resource_group_name, vm_scale_set_name, instance_id, custom_headers = nil)
      ...
      promise = promise.then do |result|
        ...
        if status_code == 200
          begin
            parsed_response = response_content.to_s.empty? ? nil : JSON.load(response_content)
            result.body = Azure::ARM::Compute::Models::VirtualMachineScaleSetVMInstanceView.new.from_json(parsed_response)
          rescue Exception => e
            fail MsRest::DeserializationError.new('Error occurred in deserializing the response', e.message, e.backtrace, result)
          end
        end
    ...
    def list_async(resource_group_name, virtual_machine_scale_set_name, filter = nil, select = nil, expand = nil, custom_headers = nil)
      ...
      promise = promise.then do |result|
        ...
        if status_code == 200
          begin
            parsed_response = response_content.to_s.empty? ? nil : JSON.load(response_content)
            result.body = Azure::ARM::Compute::Models::VirtualMachineScaleSetVMListResult.new.from_json(parsed_response)
          rescue Exception => e
            fail MsRest::DeserializationError.new('Error occurred in deserializing the response', e.message, e.backtrace, result)
          end
        end
