...
module Azure::Batch::V2019_06_01_9_0
  ...
  class CertificateOperations
    ...
    def add_async(certificate, certificate_add_options:nil, custom_headers:nil)
      ...
      # Serialize Request
      request_mapper = Azure::Batch::V2019_06_01_9_0::Models::CertificateAddParameter.mapper()
      ...
      promise = promise.then do |result|
        ...
        if status_code == 200
          begin
            ...
            result_mapper = Azure::Batch::V2019_06_01_9_0::Models::CertificateListResult.mapper()
    ...
    # not installed on any Nodes (even if you remove a Certificate from a Pool, it
    # is not removed from existing Compute Nodes in that Pool until they restart),
    # and no running Tasks depend on the Certificate. If you try to delete a
    # Certificate that is in use, the deletion fails. The Certificate status
    # changes to deleteFailed. You can use Cancel Delete Certificate to set the
    # status back to active if you decide that you want to continue using the
    # Certificate.
    #
    # @param thumbprint_algorithm [String] The algorithm used to derive the
    # thumbprint parameter. This must be sha1.
    ...
    request_url = request_url.gsub('{batchUrl}', @client.batch_url)
      ...
      promise = promise.then do |result|
        ...
        if status_code == 200
          begin
            parsed_response = response_content.to_s.empty? ? nil : JSON.load(response_content)
            result_mapper = Azure::Batch::V2019_06_01_9_0::Models::Certificate.mapper()
            result.body = @client.deserialize(result_mapper, parsed_response)
          rescue Exception => e
            fail MsRest::DeserializationError.new('Error occurred in deserializing the response', e.message, e.backtrace, result)
          end
