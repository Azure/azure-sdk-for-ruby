...
module Microsoft.VMwareCloudSimple
  ...
  class VMwareCloudSimpleClient < MsRestAzure::AzureServiceClient
      ...
      promise = promise.then do |result|
        ...
        if status_code == 200
          begin
            ...
            result_mapper = Microsoft.VMwareCloudSimple::Models::OperationResource.mapper()
    ...
    def get_private_cloud_async(pc_name, custom_headers = nil)
      ...
      promise = promise.then do |result|
        ...
        if status_code == 200
          begin
            parsed_response = response_content.to_s.empty? ? nil : JSON.load(response_content)
            result_mapper = Microsoft.VMwareCloudSimple::Models::PrivateCloud.mapper()
            result.body = self.deserialize(result_mapper, parsed_response)
          rescue Exception => e
            fail MsRest::DeserializationError.new('Error occurred in deserializing the response', e.message, e.backtrace, result)
          end
    ...
    # @return [OperationResource] operation results.
    #
    def begin_get_operation_result_by_region(operation_id, custom_headers = nil)
      response = begin_get_operation_result_by_region_async(operation_id, custom_headers).value!
      response.body unless response.nil?
    end
    ...
    # @return [MsRestAzure::AzureOperationResponse] HTTP response information.
    #
    def begin_get_operation_result_by_region_with_http_info(operation_id, custom_headers = nil)
      begin_get_operation_result_by_region_async(operation_id, custom_headers).value!
    end
    ...
    # @return [Concurrent::Promise] Promise object which holds the HTTP response.
    #
    def begin_get_operation_result_by_region_async(operation_id, custom_headers = nil)
      fail ArgumentError, 'api_version is nil' if api_version.nil?
      fail ArgumentError, 'subscription_id is nil' if subscription_id.nil?
      fail ArgumentError, 'region_id is nil' if region_id.nil?
      fail ArgumentError, 'referer is nil' if referer.nil?
    ...
    def add_telemetry
        ...
        if defined? Microsoft.VMwareCloudSimple::VERSION
          sdk_information = "#{sdk_information}/#{Microsoft.VMwareCloudSimple::VERSION}"
        end
