#-------------------------------------------------------------------------
# Copyright (c) Microsoft. All rights reserved.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#   http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
#--------------------------------------------------------------------------
require "test_helper"
require 'azure/service_bus/service_bus'

describe Azure::ServiceBus::ServiceBus do
  subject { Azure::ServiceBus::ServiceBus.new }
  
  describe '#create_queue' do
    it '' do
      
    end
  end
  describe '#delete_queue' do
    it '' do
      
    end
  end
  describe '#get_queue' do
    it '' do
      
    end
  end
  describe '#list_queues' do
    it '' do
      
    end
  end
  describe '#create_topic' do
    it '' do
      
    end
  end
  describe '#delete_topic' do
    it '' do
      
    end
  end
  describe '#get_topic' do
    it '' do
      
    end
  end
  describe '#list_topics' do
    it '' do
      
    end
  end
  describe '#create_rule' do
    it '' do
      
    end
  end
  describe '#delete_rule' do
    it '' do
      
    end
  end
  describe '#get_rule' do
    it '' do
      
    end
  end
  describe '#list_rules' do
    it '' do
      
    end
  end
  describe '#create_subscription' do
    it '' do
      
    end
  end
  describe '#delete_subscription' do
    it '' do
      
    end
  end
  describe '#get_subscription' do
    it '' do
      
    end
  end
  describe '#list_subscriptions' do
    it '' do
      
    end
  end
  describe '#send_topic_message' do
    it '' do
      
    end
  end
  describe '#peek_lock_subscription_message' do
    it '' do
      
    end
  end
  describe '#unlock_subscription_message' do
    it '' do
      
    end
  end
  describe '#read_delete_subscription_message' do
    it '' do
      
    end
  end
  describe '#delete_subscription_message' do
    it '' do
      
    end
  end
  describe '#send_queue_message' do
    it '' do
      
    end
  end
  describe '#peek_lock_queue_message' do
    it '' do
      
    end
  end
  describe '#unlock_queue_message' do
    it '' do
      
    end
  end
  describe '#read_delete_queue_message' do
    it '' do
      
    end
  end
  describe '#delete_queue_message' do
    it '' do
      
    end
  end
  describe '#receive_queue_message' do
    it '' do
      
    end
  end
  describe '#receive_subscription_message' do
    it '' do
      
    end
  end
end