...
module AzureNetworkManagement
  ...
  class ApplicationGateways
      ...
      promise = promise.then do |http_response|
        ...
        if status_code == 200
          begin
            ...
            if (parsed_response)
              parsed_response = AzureNetworkManagement::Models::ApplicationGateway.deserialize_object(parsed_response)
            end
    ...
    def begin_create_or_update(resource_group_name1, application_gateway_name1, parameters1, custom_headers = nil)
      ...
      if (parameters1)
        parameters1 = AzureNetworkManagement::Models::ApplicationGateway.serialize_object(parameters1)
      end
      ...
      promise = promise.then do |http_response|
        ...
        if status_code == 200
          begin
            parsed_response = JSON.load(response_content) unless response_content.to_s.empty?
            if (parsed_response)
              parsed_response = AzureNetworkManagement::Models::ApplicationGatewayListResult.deserialize_object(parsed_response)
            end
            result.body = parsed_response
          rescue Exception => e
            fail MsRest::DeserializationError.new("Error occured in deserializing the response", e.message, e.backtrace, response_content)
