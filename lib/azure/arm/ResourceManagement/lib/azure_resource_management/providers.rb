      ...
      promise = promise.then do |http_response|
        ...
        if status_code == 200
          begin
            ...
            if (parsed_response)
              parsed_response = Azure::ARM::Resources::Models::Provider.deserialize_object(parsed_response)
            end
      ...
      promise = promise.then do |http_response|
        ...
        if status_code == 200
          begin
            parsed_response = JSON.load(response_content) unless response_content.to_s.empty?
            if (parsed_response)
              parsed_response = Azure::ARM::Resources::Models::ProviderListResult.deserialize_object(parsed_response)
            end
            result.body = parsed_response
          rescue Exception => e
            fail MsRest::DeserializationError.new("Error occured in deserializing the response", e.message, e.backtrace, response_content)
