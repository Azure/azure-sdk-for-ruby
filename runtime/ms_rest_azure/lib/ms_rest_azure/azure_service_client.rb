...
module MsRestAzure
  ...
  class AzureServiceClient < MsRest::ServiceClient
    ...
    def get_long_running_operation_result(azure_response, custom_deserialization_block, final_state_via = FinalStateVia::DEFAULT)
      ...
      unless AsyncOperationStatus.is_terminal_status(polling_state.status)
            ...
            if !polling_state.azure_async_operation_header_link.nil?
              ...
              if final_state_via == FinalStateVia::LOCATION
                if !polling_state.response.body.to_s.empty?
                  ...
                  raise AzureOperationError, 'Location header is missing from long running operation'
                end
              else
                raise AzureOperationError, 'Location header is missing from long running operation'
              end
        ...
        raise polling_error if polling_error.is_a?(Exception)
      end
      ...
      if AsyncOperationStatus.is_failed_status(polling_state.status)
        raise polling_state.get_operation_error
      end
    ...
    def check_for_status_code_failure(azure_response)
      raise MsRest::ValidationError, 'Azure response cannot be nil' if azure_response.nil?
      raise MsRest::ValidationError, 'Azure response cannot have empty response object' if azure_response.response.nil?
      raise MsRest::ValidationError, 'Azure response cannot have empty request object' if azure_response.request.nil?

      status_code = azure_response.response.status
      http_method = azure_response.request.method

      raise AzureOperationError, "Unexpected polling status code from long running operation #{status_code}" unless status_code === 200 || status_code === 202 ||
    ...
    def update_state_from_get_resource_operation(request, polling_state, custom_deserialization_block)
      ...
      raise AzureOperationError, 'The response from long running operation does not contain a body' if result.response.body.nil? || result.response.body.empty?
    ...
    def update_state_from_location_header(request, polling_state, custom_deserialization_block, final_state_via = FinalStateVia::DEFAULT)
      ...
      if status_code === 202
        ...
        raise AzureOperationError, "The response from long running operation does not have a valid status code. Method: #{http_method}, Status Code: #{status_code}"
      end
    end
    ...
    def update_state_from_azure_async_operation_header(request, polling_state)
      ...
      raise AzureOperationError, 'The response from long running operation does not contain a body' if result.body.nil? || result.body.status.nil?
    ...
    def get_async_with_custom_deserialization(request, custom_deserialization_block)
      ...
      if !result.body.nil? && !custom_deserialization_block.nil?
        begin
          ...
          raise MsRest::DeserializationError.new('Error occured in deserializing the response', e.message, e.backtrace, http_response.body)
        end
      end
    ...
    def get_async_common(request)
      ...
      request.headers['x-ms-client-request-id'] = SecureRandom.uuid unless request.headers.key?('x-ms-client-request-id')
      request.headers['Content-Type'] = 'application/json' unless request.headers.key?('Content-Type')
      ...
      if status_code != 200 && status_code != 201 && status_code != 202 && status_code != 204
        ...
        raise AzureOperationError.new request, http_response, error_data, "Long running operation failed with status #{status_code}"
      end
      ...
      begin
        ...
        raise MsRest::DeserializationError.new('Error occured in deserializing the response', e.message, e.backtrace, result)
      end
