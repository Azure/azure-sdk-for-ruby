...
module MsRestAzure
  describe PollingState do
    it 'should initialize status from flattened response body' do
      ...
      response = double('response', status: 201, headers:           { 'Azure-AsyncOperation' => 'async_operation_header',
    ...
    it 'should grab timeout from constructor' do
      response = double('response', headers:                                       { 'RetryAfter' => 5 },
                                    status: 204)
    ...
    it 'should grab timeout from response' do
      response = double('response', headers:                                       { 'Retry-After' => 5 },
                                    status: 204)
    ...
    it 'should return succeeded status for a 200 response and no provisioning status' do
      response = double('response', status: 200, headers:           { 'Azure-AsyncOperation' => 'async_operation_header',
                                                                      'Location' => 'location_header' })
      azure_response = double('response',
                              request: nil,
                              response: response,
