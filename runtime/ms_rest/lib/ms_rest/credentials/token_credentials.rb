    ...
    def initialize(*args)
      if args.size == 1
        if args[0].respond_to? :get_authentication_header
          ...
          raise ArgumentError, 'Invalid argument was passed, is can be either TokenProvider or token'
        end
      elsif args.size == 2
        token = args[0]
        token_type = args[1]
        ...
        raise ArgumentError, 'Invalid number of parameters was passed to TokenCredentials constructor, valid number is 1 or 2'
      end
    end
